...
A collection of premium, single-purpose keepsake website templates.
Each project lives in its own folder and is fully self-contained —
its own `index.html`, its own README, and (where written) its own
Etsy listing copy.
...
| [`trousseau/`](trousseau/) | **Trousseau** | A bright stationery-suite anniversary keepsake. |
| [`kept/`](kept/) | **Kept** | Keepsake template (see its README). |
| [`perihelion/`](perihelion/) | **Perihelion** | Keepsake template (see its README). |
| [`our-story/`](our-story/) | **Our Story** | A cinematic scrolling anniversary experience — in active development. |

## Live previews

The repository is published with GitHub Pages. Each project is viewable at:

`https://zaynabaaay.github.io/Etsy/<folder>/`

The root URL serves a simple hub page linking to every project.
